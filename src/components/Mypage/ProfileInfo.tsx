...
const InfoCard: React.FC<InfoCardProps> = ({ head, body }) => (
  <div className={BUNDLE_CSS}>
    ...
    {head === 'Bio.' ? <Link className="bg-transparent underline" to={body}><span className={BODY_CSS}>{body}</span></Link> : <span className={BODY_CSS}>{body}</span>}
  </div>
);

const ProfileInfo = () => {
  return (
    <div className='profile-info bg-transparent flex flex-row mt-10'>
      <div className='bg-transparent flex flex-row'>
        <img className='bg-white w-36 h-36 rounded-full ml-10 mr-10' src={IMG_URL} alt='fake-avartar' />
        <div className='bg-transparent mr-40'>
